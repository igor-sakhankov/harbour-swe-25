        ...
        for (Field field : object.getClass().getDeclaredFields()) {
            if (field.isAnnotationPresent(InjectProperty.class)) {
                ...
                if (field.getAnnotation(InjectProperty.class).value().isBlank()) {
                    field.set(object, readProperties(field.getName()).get(field.getName()));
                } else {
                    field.set(object, readProperties(object).get(field.getAnnotation(InjectProperty.class).value()));
                }
            }
        }
    }
